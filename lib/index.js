...
function _pre(value) {
  return `${chalk.green(value)}${new Array(6 - value.length).join(' ')}`;
}
  ...
  .then(() => {
    ...
    return _actions.reduce((prev, a) => {
      ...
      const _getTemplate = () => {
        ...
        throw new Error(`Invalid template, given '${a.template}'`);
      };

      const _sourceFiles = () => {
        return fs.statSync(_src).isDirectory()
          ? glob.sync(`${_src}/**/*`, { dot: true })
          : [_src];
        };
        ...
        switch (a.type) {
          ...
          case 'modify':
            ...
            return _askIf($.io, options.force !== true,
              ...
              logger(`  ${_pre(result === 'skip' ? 'skip ' : a.type)}  ${path.relative($.cwd, _dest)}`);
            });
