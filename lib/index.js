...
function _prompt(opts, cb) {
  ...
  process.nextTick(() => {
    (ui.output || process.stdout).write(_.CLR);
  });
}
...
function _install($, task, logger, quietly, destPath) {
  ...
  _.DEPS.forEach(key => {
    ...
    tasks.push(() => {
      ...
      return logger('execute', args.join(' '), end =>
        _exec($.io, args, destPath)
          .then(data => {
            ...
            if (!end) {
              logger.write(`${data.replace(/\n+$/, '')}\n`);
            } else if (!quietly) {
              end(null, () => logger.write(`${data.replace(/\n+$/, '')}\n`));
            } else {
              end();
            }
          }));
...
function _runTask($, task, logger) {
        ...
        return prev.then(() => {
          ...
          switch (a.type) {
            case 'copy': {
              ...
              return options.copy !== false && _sourceFiles().reduce((_prev, cur) =>
                _prev.then(() => {
                  ...
                  return logger('write', path.relative($.cwd, _dest), () =>
                    _askIf($.io,
                    ...
                    .then(result => {
                      ...
                      if (_skipAll || result === 'skip') {
                        return ['skip', path.relative($.cwd, _dest)];
                      }

                      return path.relative($.cwd, _dest);
                    }));
                }), Promise.resolve());
            }
            ...
            case 'modify':
              ...
              return logger('change', path.relative($.cwd, _dest), () => {
                /* istanbul ignore else */
            ...
            case 'clone':
              ...
              return options.clone !== false && logger('clone', _src, () => _askIf($.io, (fs.existsSync(_dest)
              ...
              .then(result => new Promise((resolve, reject) => {
                ...
                /* istanbul ignore else */
                if (result === 'skip') {
                  resolve(['skip', path.relative($.cwd, _dest) || '.']);
                  return;
                }
            ...
            case 'add':
              ...
              return options.add !== false && logger('write', path.relative($.cwd, _dest), () =>
              ...
              return options.exec !== false && logger('exec', a.command, end =>
                _exec($.io, a.command)
                  .then(result => {
                    ...
                    if (!end) {
                      logger.write(`${result.replace(/\n+$/, '')}\n`);
                    } else if (!(options.quiet || a.quiet)) {
                      end(null, () => logger.write(`${result.replace(/\n+$/, '')}\n`));
                    } else {
                      end();
                    }
                  }));
  ...
  if (task.quiet || options.quiet) {
    ...
    // bypass everything but the given callback
    ...
    // backup, wrap and restore given logger
    return _logger('Executing tasks', cb =>
      run().then(() => cb && cb()));
  }
...
module.exports = function Haki(cwd, options) {
  ...
  const _logger = options.logger
    /* eslint-disable no-nested-ternary */
    || (l.setLogger(options.stdout, options.stderr)
  ...
  return {
    ...
    runGenerator(name, defaults) {
      ...
      if (typeof name === 'object') {
        return Promise.resolve().then(() =>
          _runTask.call(this, { cwd, io: _config, options, defaults }, name, _logger));
      }
