...
function _runTask(cwd, task, defaults, overwrite) {
  ...
  if (task.title) {
    const max = task.title.length;
    const sep = new Array(max + 1).join('-');
  ...
  return task.prompts.reduce((prev, p) =>
    prev.then(() => new Promise((resolve, reject) => {
      ...
      if (p.type === 'choose') {
        ...
        /* istanbul ignore else */
        if (p.message) {
          _echo(chalk.gray(p.message), '\n');
        }

        p.options.forEach((o, i) => {
          if (typeof o === 'object') {
            _echo(chalk.gray(`${i + 1})`), ` ${chalk.cyan(o.name)}`,
              o.detail ? `  ${chalk.gray(o.detail)}` : '',
              '\n');
          } else {
            _echo(chalk.gray(`${i + 1})`), ` ${chalk.cyan(o)}\n`);
          }
        });

        // choose prompter
        promptly[p.type](chalk.gray('›'), p.options.map(o => o.value || o), _params, _end);
      } else {
        // generic prompter
        promptly[p.type](chalk.gray(`› ${p.message}`), _params, _end);
      }
    }))
  , Promise.resolve())
  .then(() => {
...
module.exports = function Haki(cwd) {
  ...
  return {
    ...
    chooseGeneratorList() {
      ...
      return _runTask.call(this, cwd, {
        title: 'Generators available:',
        prompts: [{
          name: 'task',
          type: 'choose',
          // message: 'Choose a generator:',
          options: this.getGeneratorList().map(o => ({
            name: o.name,
            value: o.name,
            detail: o.task.description,
          })),
        }],
        actions: [(input) => {
          _task = input.task;
      ...
      .then((result) => {
        /* istanbul ignore else */
        if (result.error) {
          _echo(chalk.red(result.error));
        }

        return _runTask.call(this, cwd, _tasks[_task]);
      });
    }
  };
};
