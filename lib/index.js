      ...
      queue.push(() => new Promise((ok, fail) => {
        ...
        if (x.input.length > 0) {
          ...
          field.choices = [
            ...
            { title: 'object', value: 'obj' },
            { title: 'array', value: 'arr' },
          ];
        }
        ...
        _prompt($, [field])
          .then(response => {
            /* istanbul ignore else */
            if (field.type === 'select' && response[x.key] === 'bool') {
              return _prompt($, [{ name: x.key, type: 'confirm' }]);
            }
            return response;
          })
          .then(response => {
  ...
  srcFiles.reduce((prev, file) => {
    return prev.then(() => _promptly($, logger, _values, _tags(file, file.replace($.cwd, '.'))).then(_vars => {
      ...
      return Promise.resolve()
        .then(() => {
          /* istanbul ignore else */
          if (exts.includes(path.extname(file).substr(1))) {
