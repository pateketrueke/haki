  ...
  DEPS.forEach(key => {
    ...
    tasks.push(() =>
      new Promise((resolve, reject) => {
        /* istanbul ignore else */
        if (args.length) {
          args.unshift($.options.yarn === true ? 'add' : 'install');
        }
        ...
        const child = cp.spawn(cmd, args, { cwd: destPath || $.cwd });
    ...
    return _actions.reduce((prev, a) => {
      ...
      return prev.then(() => {
        ...
        switch (a.type) {
          ...
          case 'install':
            return _install($, a, logger, _destPath());
