...
function _prompt({ enquirer }, input) {
  // pause earlier
  lp.pause();
    ...
    .then(response => {
      ...
      const out = input.reduce((prev, cur) => {
        ...
        if (typeof response[cur.name] !== 'undefined') {
          prev[cur.name] = cur.choices
            ? cur.choices.find(x => x.result === response[cur.name]
              || x.value === response[cur.name]
              || x.name === response[cur.name]).result
            : response[cur.name];
        }
...
module.exports = function Haki(cwd, options) {
  Enquirer = Enquirer || require('enquirer');
  ...
  return {
    ...
    prompt(opts) {
      ...
      return _prompt({ enquirer, haki: this }, !Array.isArray(opts) ? [opts] : opts);
    },
    ...
    chooseGeneratorList(defaults) {
      ...
      return new Promise((resolve, reject) => {
        _prompt({ enquirer, haki: this }, [{
          ...
          resolve(this.runGenerator(task, defaults));
        });
      });
    },
  };
