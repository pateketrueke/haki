...
function _prompt(opts, cb) {
  ...
  try {
    ...
    cb(new Error(`Unsupported '${_type}' prompt. ${e.message}`));
    return;
  }
  ...
  if (opts.choices) {
    opts.choices = opts.choices.map(c => {
      c = typeof c === 'string' ? { name: c, result: c } : c;
      c.name = c.label || c.name;
      c.result = c.value || c.result;
      delete c.value;
      delete c.label;
      return c;
    });
...
function _askIf(io, err, label, options, skipFlag) {
  return new Promise((resolve, reject) => {
    /* istanbul ignore else */
    if (!err) {
      resolve();
...
function _install($, task, logger, quietly, destPath) {
  ...
  _.DEPS.forEach(key => {
    ...
    tasks.push(() => {
      ...
      return logger('install', _deps.join(' '), end => _exec($.io, args, destPath)
        .then(data => {
          /* istanbul ignore else */
          if (task[key]) {
            result[key] = task[key];
...
function _runTask($, task, logger, _helpers) {
  ...
  // main
  const run = () => _prompts.reduce((prev, p) => {
    /* istanbul ignore else */
    if (!p) {
      return null;
    }
    ...
    return prev.then(() => new Promise((resolve, reject) => {
      ...
      if (_values[p.name] && _values[p.name].length) {
        const test = typeof p.validate === 'function' ? p.validate(_values[p.name]) : true;

        /* istanbul ignore else */
        if (test !== true) {
          throw new Error(test || `Invalid input for '${p.name}'`);
        }
          ...
          switch (a.type) {
            case 'copy': {
              ...
              return options.copy !== false && _sourceFiles().reduce((_prev, cur, i) => _prev.then(() => {
                ...
                return logger('write', path.relative($.cwd, _dest), end => _askIf($.io,
                  ...
                  .then(result => {
                    /* istanbul ignore else */
                    if (result === 'abort') {
                      throw new Error(`Source '${path.relative($.cwd, _src[i])}' cannot be copied!`);
                    }
            ...
            case 'add':
              ...
              return options.add !== false && logger('write', path.relative($.cwd, _dest), end => _askIf($.io, fs.existsSync(_dest) && options.force !== true,
                `Replace '${path.relative($.cwd, _dest)}'`,
                _.SINGLE_REPLACE_CHOICES, skipMe)
                .then(result => {
                  /* istanbul ignore else */
            ...
            case 'exec':
              ...
              return options.exec !== false && logger('exec', a.command, end => _exec($.io, a.command)
                .then(result => {
                  _changes.push({
                    type: a.type,
                    stdOut: result,
            ...
            case 'clean':
              ...
              return logger('clean', path.relative($.cwd, _dest), end => _askIf($.io, options.force !== true,
                `Delete '${path.relative($.cwd, _dest) || '.'}'`,
                _.SINGLE_DELETE_CHOICES).then(result => {
                /* istanbul ignore else */
                if (result === 'abort') {
                  throw new Error(`Output '${path.relative($.cwd, _dest) || '.'}' cannot be destroyed!`);
                }
  ...
  return {
    ...
    runGenerator(name, defaults) {
      ...
      if (typeof name === 'object') {
        return Promise.resolve().then(() => _runTask.call(this, {
...
module.exports.closeAll = () => {
  _readlines.splice(0, _readlines.length).forEach(old => {
    /* istanbul ignore else */
    if (old && old.prompter && old.prompter.close) {
      old.prompter.close();
      old.ui.close();
    }
  });
