...
function _exec(io, task, logger) {
  return new Promise((resolve, reject) => {
    const env = task.env || {};

    const ps = cp.exec(task.command, { env });
    ...
    ps.on('close', (code) => {
      if (code === 0) {
        logger(`  ${_pre(task.type)}  ${task.command}${CLR}\n`);
        resolve();
      } else {
...
function _install($, task, logger) {
  return new Promise((resolve, reject) => {
    const deps = task.dependencies || task.devDependencies || task.optionalDependencies || [];
    const cmd = $.options.yarn === true ? 'yarn' : 'npm';

    if (deps.length) {
      deps.unshift($.options.yarn === true ? 'add' : 'install');
    }

    if (task.devDependencies) {
      deps.push(`--${$.options.yarn === true ? 'dev' : 'save-dev'}`);
    } else {
      if ($.options.yarn !== true) {
        deps.push(`--save${task.optionalDependencies ? '-optional' : ''}`);
      }
    }

    logger(`${chalk.gray(`↺ Installing dependencies with ${cmd}...`)}${CLR}\r`);

    const child = cp.spawn(cmd, deps, { cwd: $.cwd });

    child.on('error', reject);
    child.on('close', () => {
      logger(`  ${_pre(cmd)}  ${path.relative($.cwd, 'node_modules')}${CLR}\n`);
      resolve();
    });
  });
}
      ...
      return prev.then(() => {
        ...
        switch (a.type) {
          ...
          case 'modify':
            ...
            return _askIf($.io, options.force !== true,
              ...
              if (!result || result === 'continue') {
                ...
                fs.outputFileSync(_dest, content
          ...
          case 'exec':
            ...
            return _exec($.io, a, logger);

          case 'install':
            return _install($, a, logger);
