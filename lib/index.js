        ...
        try {
          ...
          switch (a.type) {
            ...
            case 'clone':
              ...
              /* istanbul ignore else */
              if (!(a.gitUrl && a.gitUrl.indexOf('/') > 1)) {
                throw new Error(`Missing repository source, given ${a.gitUrl}`);
              }

              const projectPath = path.join(dest, _render(a.gitUrl, _values, HELPERS));
              const readdir = fs.readdirSync(projectPath);

              if (readdir.length !== 0){
                throw new Error(`Path '${path.relative($.cwd, projectPath)}' must be empty`);
              }

              downloadRepo(a.gitUrl, projectPath, (err) => {
                if (err) {
                  reject(new Error(`Repository not found https://github.com/${a.gitUrl}`));
                  return;
                }
