...
function _install($, task, logger, destPath) {
  ...
  DEPS.forEach(key => {
    ...
    tasks.push(() => {
      ...
      args.unshift($.options.yarn === true ? 'yarn' : 'npm');

      /* istanbul ignore else */
      if (args.length > 2) {
        if (key === 'devDependencies') {
          args.push(`--${$.options.yarn === true ? 'dev' : 'save-dev'}`);
      ...
      logger(`${chalk.gray(`↺ Installing ${key}...`)}${CLR}\r`);

      return _exec($.io, args, true, destPath)
        .then(() => {
          /* istanbul ignore else */
          if (task[key]) {
            result[key] = task[key];
...
function _runTask($, task, logger) {
  ...
  .then(() => {
    ...
    logger(`${chalk.gray(`↺ Loading ${_actions.length} task${_actions.length === 1 ? '' : 's'}...`)}${CLR}\r`);
        ...
        switch (a.type) {
            ...
            return _sourceFiles().reduce((_prev, cur) =>
              _prev.then(() => {
                ...
                return _askIf($.io,
                  ...
                  logger(`  ${chalk.green(_skipAll || result === 'skip' ? 'skip' : 'save')}  ${path.relative($.cwd, _dest)}${CLR}\n`);
                });
              }), Promise.resolve());
          ...
          case 'modify':
            ...
            return _askIf($.io, options.force !== true,
              ...
              /* istanbul ignore else */
              if (result === 'abort') {
                throw new Error(`File '${_src}' cannot be modifed!`);
              }
              ...
              if (!result || result === 'continue') {
                ...
                if (a.unless
                  ...
                  throw new Error('Modification already exists');
                }
              ...
              logger(`  ${chalk.green(result === 'skip' ? 'skip' : 'change')}  ${path.relative($.cwd, _dest)}${CLR}\n`);
            });
          ...
          case 'clone':
            ...
            return _askIf($.io, (fs.existsSync(_dest) ? fs.readdirSync(_dest).length !== 0 : false) && options.force !== true,
              `Overwrite '${path.relative($.cwd, _dest) || '.'}' by cloning '${_src}'`,
              SINGLE_CHOICES
              ...
              logger(`${chalk.gray(`↺ Downloading ${_src} from GitHub...`)}${CLR}\r`);

              downloadRepo(_src, _dest, err => {
                if (err) {
                  reject(new Error(`Repository not found https://github.com/${_src}`));
                } else {
                  logger(`  ${chalk.green('clone')}  ${path.relative($.cwd, _dest) || '.'} (${_src})${CLR}\n`);
          ...
          case 'add':
            ...
            return _askIf($.io, fs.existsSync(_dest) && options.force !== true,
              ...
              logger(`  ${chalk.green(result === 'skip' ? 'skip' : 'save')}  ${path.relative($.cwd, _dest)}${CLR}\n`);
            });
          ...
          case 'exec':
            ...
            return _exec($.io, a.command, options.quiet || a.quiet)
              .then(result => {
                _changes.push({
                  type: a.type,
                  stdOut: result,
          ...
          case 'render':
            (Array.isArray(a.dest) ? a.dest : [a.dest]).forEach(dest => {
