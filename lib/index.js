...
function _install($, task, logger, quietly, destPath) {
  ...
  _.DEPS.forEach(key => {
    ...
    tasks.push(() => {
      ...
      return logger('execute', args.join(' '), () =>
...
function _runTask($, task, logger) {
  ...
  .then(() => {
    ...
    logger.printf('\r{gray.wait|Loading %s task%s ...}\r\r', _actions.length, _actions.length === 1 ? '' : 's');
      ...
      return prev.then(() => {
        ...
        switch (a.type) {
          case 'copy': {
            ...
            return options.copy !== false && _sourceFiles().reduce((_prev, cur) =>
              _prev.then(() => {
                ...
                return logger('write', path.relative($.cwd, _src), () =>
                  _askIf($.io,
          ...
          case 'modify':
            ...
            return logger('modify', path.relative($.cwd, _dest), () => {
              /* istanbul ignore else */
          ...
          case 'exec':
            ...
            return options.exec !== false && _exec($.io, a.command)
              .then(result => {
                _changes.push({
                  type: a.type,
                  stdOut: result,
...
module.exports = function Haki(cwd, options) {
  ...
  const _logger = options.logger
    ...
    || (l.setLogger(options.stdout ? options.stdout.write.bind(options.stdout) : null)
