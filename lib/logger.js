...
function status(start, type, out, cb) {
  ...
  try {
    if (cb) {
      this.printf('\r\r  {pad.gray|%s} {wait.gray|%s ...}\r', type, src);
      retval = cb();
    }
  } catch (e) {
    err = e;
    ...
    if (ok) {
      ...
      if (err || _error) {
        this.printf('\r\r  {pad.gray|%s} {err.red|%s} %s\n', type,
          (res && Array.isArray(res) ? res[1] : res) || src || dest,
          ms);

        this.printf('{red|%s}\n', (err || _error));
      } else {
        type = (res && Array.isArray(res) ? (_.TYPES[res[0]] || res[0]) : '') || type;

        this.printf('\r\r  {pad.gray|%s} {%s|%s} %s\n',
          type,
          _.TYPES[type] || 'ok.green',
          (Array.isArray(res) ? res[1] : res) || dest,
          ms);
      }
    }
