...
function status(start, type, out, cb) {
  ...
  function begin() {
    if (type === false) {
      this.printf('\r\r{wait|%s ...}\r', src);
    } else {
      this.printf('\r\r  {pad.gray|%s} {wait|%s ...}\r', prefix || type, src);
    }
    ...
    interval = setInterval(() => {
      if (type === false) {
        this.printf('\b{gray|%s %s ...}', _.CHARS[i], src);
      } else {
        this.printf('\b  {pad.gray|%s} {gray|%s %s ...}', prefix || type, _.CHARS[i], src);
      }
